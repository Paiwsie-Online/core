...
/*            $userModel = new Yii::$app->controller->modelClass;
    ...
    public function multipleParticipants($objectType, $modelArr) {
        $query = self::find()->alias('a');
        $opCounter = 0;
        foreach ($modelArr as $model => $modelid) {
            $opCounterAlias = 'op' . $opCounter;
            $query->leftJoin('object_participant '.$opCounterAlias, $opCounterAlias.'.object = a.id')->andWhere([$opCounterAlias.'.model' => $model, $opCounterAlias.'.model_id' => $modelid]);
            $opCounter++;
        }
